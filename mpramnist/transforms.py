...
class AddFlanks(nn.Module):
    ...
    def forward(self, Seq: seqobj) -> seqobj:
        ...
        for name in Seq.vectors:
            left_flank_vect = [
                Seq.vectors[name].tp.levels[Seq.vectors[name].pad_value]
            ...
            Seq.vectors[name].val = (
                left_flank_vect + Seq.vectors[name].val + right_flank_vect
            )
...
class LeftCrop(nn.Module):
    ...
    def forward(self, Seq: seqobj) -> seqobj:
        ...
        if len(Seq.seq) < self.min:
            raise ValueError(
                f"Sequence length ({len(Seq.seq)}) is shorter than min_crop ({self.min})."
            )

        crop_coordinate = torch.randint(
            size=(1,), low=self.min, high=self.max + 1
        ).item()
...
class RightCrop(nn.Module):
    ...
    def forward(self, Seq: seqobj) -> seqobj:
        ...
        if len(Seq.seq) < self.min:
            raise ValueError(
                f"Sequence length {len(Seq.seq)} is smaller than the minimum crop size {self.min}."
            )
...
class CenterCrop(nn.Module):
    ...
    def forward(self, Seq: seqobj) -> seqobj:
        ...
        for name in Seq.vectors:
            if crop_len > 1:
                Seq.vectors[name].val = Seq.vectors[name].val[
                    crop_len // 2 + crop_len % 2 :
                ]
                Seq.vectors[name].val = Seq.vectors[name].val[
                    : -crop_len // 2 + crop_len % 2
                ]
...
class RandomCrop(nn.Module):
    ...
    def forward(self, Seq: seqobj) -> seqobj:
        ...
        crop_coordinate = torch.randint(
            size=(1,), low=0, high=Seq.seqsize - self.output_size + 1
        ).item()
        ...
        for name in Seq.vectors:
            Seq.vectors[name].val = Seq.vectors[name].val[
                crop_coordinate : crop_coordinate + self.output_size
            ]
...
class Padding(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(output_size, int) and output_size <= 0:
            raise ValueError("Output size must be a positive integer.")
        elif isinstance(output_size, tuple) and (
            len(output_size) != 2
            or not all(isinstance(i, int) and i >= 0 for i in output_size)
        ):
            raise ValueError(
                "Output size tuple must contain two non-negative integers."
            )
    ...
    def forward(self, Seq: seqobj) -> seqobj:
        if isinstance(self.output_size, int) and self.output_size - Seq.seqsize > 0:
            ...
            # change vector feature
            for name in Seq.vectors:
                pad_value = Seq.vectors[name].pad_value
                Seq.vectors[name].val = (
                    pad_value * (pad_value // 2) + Seq.vectors[name].val
                )
                Seq.vectors[name].val = (
                    Seq.vectors[name].val
        ...
        if isinstance(self.output_size, tuple):
            ...
            for name in Seq.vectors:
                Seq.vectors[name].val = (
                    Seq.vectors[name].pad_value * left_pad
                    + Seq.vectors[name].val
                    + Seq.vectors[name].pad_value * right_pad
                )
