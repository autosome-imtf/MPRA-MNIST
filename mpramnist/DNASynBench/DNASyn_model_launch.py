...
from mpramnist.DNASynBench.dataset import DNASynDataset
from mpramnist.DNASynBench.trainer import LitModel_DNASyn
...
for run in list(range(args.runs)):
    ...
    train_dataset = DNASynDataset(split="train",transform=train_transform,root=args.root)
    val_dataset = DNASynDataset(split="val",transform=val_test_transform,root=args.root)
    test_dataset = DNASynDataset(split="test",transform=val_test_transform,root=args.root)
    ...
    seq_model = LitModel_DNASyn(model=model, loss=loss, weight_decay=args.wd, lr=args.lr, print_each=1)
    ...
    seq_model = LitModel_DNASyn.load_from_checkpoint(best_model_path,model=model, loss=nn.MSELoss(), weight_decay=args.wd, lr=args.lr, print_each=1)

    forw_transform = t.Compose([t.Seq2Tensor()])
    rev_transform = t.Compose([t.ReverseComplement(1),t.Seq2Tensor(),])

    test_forw = DNASynDataset(split="test",transform=forw_transform,root=args.root,)
    test_rev = DNASynDataset(split="test",transform=rev_transform,root=args.root,)
