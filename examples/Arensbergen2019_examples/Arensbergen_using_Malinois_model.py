...
from mpramnist.models import BassetBranched # Malinois
...
general.add_argument("--result_dir",
                     type=str,
                     default = "./arensbergen_malinois.tsv")
...
for run in list(range(args.runs)):
    ...
    for genome_id in args.genome_ids:
        ...
        model = BassetBranched(input_len=600, n_outputs=len(CELL_TYPES), loss_criterion=L1KLmixed)

        seq_model = LitModel_Arensbergen_Reg(model=model, loss=nn.CrossEntropyLoss(), weight_decay=args.wd, lr=args.lr, print_each=1)
        ...
        seq_model = LitModel_Arensbergen_Reg.load_from_checkpoint(best_model_path, model=model, loss=nn.CrossEntropyLoss(), weight_decay=args.wd, lr=args.lr, print_each=1)
